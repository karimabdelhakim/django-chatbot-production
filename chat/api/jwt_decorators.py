...
#jwt_get_user_id_from_payload will be removed in the future.
#Use `jwt_get_username_from_payload_handler` instead
#from rest_framework_jwt.utils import jwt_get_user_id_from_payload
...
def authenticate_credentials(payload):
    ...
    """
    try:
        user_username = jwt_get_username_from_payload_handler(payload)

        if user_username:
            user = get_user_model().objects.get(username=user_username, is_active=True)
        else:
            msg = 'Invalid payload'
            raise exceptions.AuthenticationFailed(msg)
